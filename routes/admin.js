...
router.post("/register", async (req, res) => {
  try {
    ...
    res.json({savedUser , token:token});
  } catch (err) {
...
Admin.findOne({ email: email } , async function ( error , user){
  console.log(user) 
  if(!user)
...
router.post("/addNewTeam", async (req, res) => {
  ...
  let teamObj = await TeamAndRole.find({});
  console.log(teamObj , typeof  teamObj)
  if(teamObj.length ==0 ||  teamObj == undefined){
    let teamandrole = new TeamAndRole({
      teamNames: ['web','pro',teamName],
      roleNames: ['user','prof']
    });
    console.log("boucle")
  const teamandrolesaved = await teamandrole.save();
  console.log(teamandrolesaved)
  res.json(teamandrolesaved);
  }
else  {
